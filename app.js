...
class RoadmapApp {
        ...
        el.innerHTML = `
            ...
            <div class="stat-card stat-callout">
                ...
                ${calloutSubtitle ? `<div class="stat-callout-sub">${calloutSubtitle}</div>` :
                  (topOwners.length ? `<div class="stat-callout-sub">Top ${(this.currentVersion === 'd360' || this.currentVersion === 'service' || this.currentVersion === 'slack') ? 'record owners (eng leads)' : 'owners'}: ${topOwners.map(([n,c])=>`<span class="owner-pill">${n} · ${c}</span>`).join(' ')}</div>` : '')}
            </div>
        `;
    }
    ...
    renderFilterChips() {
        ...
        const showStages = (this.currentVersion === 'gus' || this.currentVersion === 'd360' || this.currentVersion === 'service' || this.currentVersion === 'slack') && stagesPresent.length > 0;
        ...
        const showClusters = (this.currentVersion === 'd360' || this.currentVersion === 'service' || this.currentVersion === 'slack') && clusterKeys.length > 0;
        const clusterGroup = showClusters
            ...
            : '';
        const showCF = (this.currentVersion === 'd360' || this.currentVersion === 'service' || this.currentVersion === 'slack') && (cfCounts['Customer-facing'] || cfCounts['Internal eng']);
    ...
    updateVersionInfo() {
        ...
        if (this.currentVersion === 'customer') {
            ...
            const versionNames = {
                ...
                'slack': 'GUS Live · Slack ↔ Salesforce Epics',
                'v1': 'V1 - Core Roadmap',
            ...
            } else if (this.currentVersion === 'slack' && typeof LAST_GUS_REFRESH_SLACK !== 'undefined') {
                refreshStamp = ` <span class="refresh-stamp" title="Last refreshed from GUS ADM_Epic__c">· Refreshed ${LAST_GUS_REFRESH_SLACK}</span>`;
            }
    ...
    switchVersion(version) {
        ...
        if (version === 'd360' || version === 'service' || version === 'slack') this.activeFilters.customerFacing.add('Customer-facing');
        this.searchQuery = '';
        this.filteredData = [...this.data];
    ...
    ownerChips(item) {
        ...
        if (item.version === 'd360' || item.version === 'service' || item.version === 'slack') {
    ...
    showItemDetails(id) {
        ...
        let ownerPrdHtml = '';
        const hasGUSFields = (item.version === 'gus' || item.version === 'd360' || item.version === 'service' || item.version === 'slack') && (item.scheduledBuild || item.devLead || item.designLead || item.qualityLead || item.team || item.health);
